...
class Production(object):
    ...
    def __init__(self, lhs, rhs, tag=None):
        """$
...
class PCFG(CFG):
    ...
    @classmethod
    def fromstring(cls, input, has_tags=False, encoding=None):
        """
        ...
        :param input: a grammar, either in the form of a string or else
             as a list of strings.
        """
        start, productions = read_grammar(input, standard_nonterm_parser, has_tags=has_tags,
...
_TAGS_RE = re.compile(r'( \{ ([\w\s\,\-\/\(\)\']+)? \} ) \s*', re.VERBOSE)
_TERMINAL_RE = re.compile(r'( "[^"]+" | \'[^\']+\' ) \s*', re.VERBOSE)
_DISJUNCTION_RE = re.compile(r'\| \s*', re.VERBOSE)


def _read_production(line, nonterm_parser, probabilistic=False, has_tags=False):
    """
    ...
    while pos < len(line):
        ...
        m = _TAGS_RE.match(line, pos)
        if has_tags and m:
            pos = m.end()
            tags[-1] = str(m.group(1)[1:-1])
    ...
    if probabilistic:
        return [ProbabilisticProduction(lhs, rhs, prob=probability, tag=tag)
                for (rhs, probability, tag) in zip(rhsides, probabilities, tags)]
    else:
        return [Production(lhs, rhs, tag) for (rhs, tag) in zip(rhsides, tags)]
...
def read_grammar(input, nonterm_parser, probabilistic=False, has_tags=False, encoding=None):
    """
    ...
    for linenum, line in enumerate(lines):
        ...
        try:
            if line[0] == '%':
                ...
                productions += _read_production(line, nonterm_parser, probabilistic, has_tags)
